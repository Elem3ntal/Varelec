    ...
    public partial class FacturaLiberada : Form
    {
        ...
        public void cargarCombobox()
        {
            foreach (string[] producto in productos)
            {
                ...
                comboBoxDescripcion.Items.Add(producto[3]);
            }
        }
        public void cargarClientes()
        {
            padre.servidor.clientesObtener().Save("clientes.dbxml");
            ...
            foreach (XmlNode nodo in nuevo.DocumentElement.LastChild.ChildNodes)
            {
                ...
                foreach (XmlNode nodo2 in nodo.ChildNodes)
                {
                    if (nodo2.Name == "RUT")
                        ...
                        cliente[3] = nodo2.InnerText;
                    else if (nodo2.Name == "comuna")
                        ...
                        cliente[7] = nodo2.InnerText;
                    else if (nodo2.Name == "ultimaFacturaCompra")
                        cliente[8] = nodo2.InnerText;
                }
        ...
        {
            padre.servidor.inventarioObtener().Save("inventario.dbxml");
            XmlDocument nuevo = new XmlDocument();
        ...
        private void buttonImprimirFactura_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in listViewFactura.Items)
            {
                foreach (string[] producto in productos)
                {
                    ...
                    {
                        producto[5] = "" + (Convert.ToInt32(producto[5]) - Convert.ToInt32(Math.Round(Convert.ToDouble(item.Text),0)));
                        padre.servidor.inventarioModificar(producto);
                        if (padre.servidor.ingresoPMP(producto[0], 0, 0, Convert.ToInt32(producto[5]), Convert.ToInt32(Convert.ToDouble(producto[6]) * 100), Convert.ToInt32(Convert.ToDouble(producto[7]) * 100), producto[12]) == false)
                        {
        ...
        private void mandarAFrecia()
        {
            ...
            if (padre.servidor.ingresarFacturaLiberada(factura))
            {
        ...
        private void comboBoxDescripcion_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (string[] producto in productos)
            {
                if (comboBoxDescripcion.SelectedItem.ToString() == producto[3])
                {
                    textBoxDescripcion.Text = comboBoxDescripcion.SelectedItem.ToString();
                    textBoxProCant.Text = producto[5];
                    textBoxPrecioUnitario.Text = producto[8];
        ...
        private void comboBoxSeñores_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (string[] cliente in clientes)
            {
                if (cliente[1] == comboBoxSeñores.SelectedItem.ToString())
                {
                    ...
                    textBoxDireccion.Text = cliente[3];
                    textBoxCiudad.Text = cliente[4] + " - " + cliente[5];
        ...
        private void comboBoxNuParte_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (string[] producto in productos)
            {
                ...
                {
                    textBoxDescripcion.Text = producto[3];
                    textBoxProCant.Text = producto[5];
                    textBoxPrecioUnitario.Text = producto[8];
                    textBoxNP.Text = producto[0];
                    comboBoxDescripcion.SelectedItem = producto[3];
                    break;
                }
            }
        }
