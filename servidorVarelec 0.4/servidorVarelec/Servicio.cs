...
namespace servidorVarelec
{
    ...
    public class Servicio:IServicio
    {
        ...
        public string consultarActualizacion()
        {
            ...
            {
                XmlDocument file = new XmlDocument();
                file.Load(folder + "/programa/Inventario.XML");
        ...
        public bool subirActualizacionAServidor()
        {
            ...
            try
            {
                ...
                FileInfo[] archivosViejos = new DirectoryInfo(rutaCarpetaPublica).GetFiles("*.*");
                foreach (FileInfo borrar in archivosViejos)
                {
                    borrar.Delete();
                }
                new WebClient().DownloadFile("http://25.108.141.130/ActualizadorVarelec/Inventario.XML", rutaCarpetaPublica + "Inventario.XML");
                ...
                {
                    string nombreArchivo = archivo.InnerText.Replace("http://25.108.141.130/ActualizadorVarelec/", "");
                    ...
                    archivo.InnerText = archivo.InnerText.Replace("25.108.141.130", "25.109.196.97");
                }
                data2.Save(rutaCarpetaPublica + "Inventario.XML");
                return true;
            }
            catch
            {
        ...
        public bool registrarInicio(string usuario, string dateComplete)
        {
            ...
            {
                string folder = Environment.CurrentDirectory;//folder+"
                if (File.Exists(folder + "/HistorialInicios.xml") == false)
                {
