...
namespace Inventario
{
    ...
    {
        private void facturaPendienteMandar()
        {
            if (revisarXMLIngresarFactura())//en caso de que no exista una factura de ingreso pendiente
            {
                DialogResult result = MessageBox.Show("Existe Factura Pendiente:\nFecha: \nProveedor: \nItems: Monto\n Enviar? el no enviarla borrará la factura guardada", "Factura de Ingreso Pendiente", MessageBoxButtons.YesNo);
                ...
                else if (result == DialogResult.No)
                {
                    ...
                    MessageBox.Show("Factura de Ingreso Pendiente ha sido eliminada correctamente");
                }
            }
        }
                    ...
                    foreach (XmlNode nodoMedio in nodo.ChildNodes)
                    {
                        ...
                        foreach (XmlNode nodo2 in nodoMedio.ChildNodes)
                        {
                            ...
                            else if (nodo2.InnerText.Equals("Nuevo"))
                            {
                                ...
                                int retorno=servidor.inventarioAgregar(producto);
                                if (retorno == 1)
                                {//Convert.ToInt32(Convert.ToDouble(costoFTR) * 100), Convert.ToInt32(Convert.ToDouble(costoExt) * 100)
                                    ...
                                    MessageBox.Show("producto NO ha podido ser ingresado - "+producto[0]+"\nCodigo de error: "+retorno);
                            }
                                ...
                                foreach (string[] producto in productos)
                                {
                                    if (producto[0].Equals(nuparte))
                                    {
                                        ...
                                        if (retorno == 1)
                                        {
                                            ...
                                            MessageBox.Show("producto NO ha podido ser modificado - " + producto[0] + "\nCodigo de error: " + retorno);
                                        break;
                                    }
                                }
                            }
